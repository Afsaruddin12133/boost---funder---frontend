...
import { AlertCircle, Check, ChevronRight, Clock, Crown, Rocket, Zap } from "lucide-react";
    ...
    <div className="w-full pb-6 space-y-6 md:space-y-8 animate-in fade-in duration-1000 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/5 pb-4 md:pb-6">
        ...
        <div className="w-full sm:w-auto mt-4 sm:mt-0 relative z-20">
          ...
          <div className="relative">
            <input 
              ...
              onChange={(e) => {
                ...
                if (!isNaN(num) && num > 0) {
                  setFundingGoalInput(num.toLocaleString('en-US'));
                } else if (val === '') {
                  setFundingGoalInput('');
                } else {
          ...
          <p className="text-[9px] text-white/30 mt-2 font-medium tracking-wide">
            {dealIdFromUrl ? "Goal amount fixed from your deal." : "Enter goal to preview estimated fees"}
          </p>
        </div>
      </div>

      {/* Pricing Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 xl:gap-6 relative pb-10">
        {packages.map((pkg) => (
          <FounderPackageCard
            key={pkg._id}
            pkg={pkg}
            ...
            calculatedFee={calculatedFees[pkg._id]}
            isCalculatingFee={isCalculatingFees}
          />
        ))}
        {packages.length === 0 && (
          <div className="col-span-full py-12 text-center border border-white/5 rounded-3xl bg-white/[0.02]">
            <p className="text-white/40">No founder packages currently available.</p>
          </div>
        )}
      </div>
      ...
      <PaymentMethodModal 
        ...
        planName={pendingPackage?.name || "Premium"}
      />
    </div>
  );
}
